...
かなレイヤーで Win(つ) / Alt(さ) をホールドしている間も、Ctrlと同じく一時的に
英数レイヤーへ移るようにした (issue #34)。<kbd>Win</kbd>+<kbd>V</kbd> などが
かな配列のまま解決されず、そのまま打てる。**タップ (つ / さ) は従来どおりで、
キー配置そのものは変わらない。**

レイヤーを下ろすのは、キーの解放イベントではなくホストへ出ている修飾の状態を
毎スキャン見て判断している (`update_hold_layer()`)。`process_record_kb()` に渡る
キーコードは押下・解放のどちらも**その時点のレイヤー**で引き直されるうえ、
英数レイヤーの つ/さ の位置は `_______` ではなく素の `KC_LGUI` / `KC_LALT` なので、
自分で英数レイヤーを上げたせいで解放が `LGUI_T(KC_Z)` として届かなくなる。
mod-tap のキーコードで解放を待つとそれが永久に来ず、英数レイヤーへ貼りついたまま
戻れなくなってしまう。修飾の状態を見れば解放がどちらのキーコードで解決されようと
必ず戻るし、英数レイヤーの並びを変えても壊れない。

あわせて Ctrl・Win・Alt の重ね押し用に、レイヤーを上げた修飾をビットで覚える
`hold_layer_on()` / `hold_layer_off()` を入れた。QMKのレイヤーは参照カウントを
持たないので、素のままだと <kbd>Ctrl</kbd>+<kbd>Win</kbd> で先に離したほうの
`layer_off()` が、まだ押している側のぶんまで落としてしまう。
