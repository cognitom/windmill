...
かなレイヤーで Win(つ) / Alt(さ) をホールドしている間も、Ctrlと同じように英数レイヤーへ
一時的に切り替わるようにした (issue #34)。かな入力のままでも <kbd>Win</kbd>+<kbd>V</kbd> や
<kbd>Alt</kbd>+<kbd>F</kbd> がアルファベットのまま打てる。**タップ (つ / さ) は従来どおり。**

修飾そのものはQMKのmod-tap (`LGUI_T(KC_Z)` / `LALT_T(KC_X)`) のままで、ホールドが確定した
ところでレイヤーを重ねるだけにしてある。あわせて、Ctrl・Win・Alt を2つ以上重ねて押した
ときのために、レイヤーの上げ下げを数えるようにした。QMKのレイヤー状態は参照カウントを
持たないので、素のままだと先に離したほうの `layer_off()` で、まだ押している側のぶんまで
レイヤーが落ちてしまう。
