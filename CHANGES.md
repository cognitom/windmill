...
`process_kana_qmark()` を単純化しようとしたが、実機で issue #18 が再発したため
取りやめた (issue #39、PR #41)。左右を揃えても、押されているShiftをいったん
外して押し直せばホストには「Shiftが離れた」ように見え、その1打鍵は取りこぼされる。
`tap_code16(S(KC_x))` に任せるとShiftのレポートと本体のレポートが間を置かずに
並ぶのも同じ理由で危ない。押されている実Shiftには触らない、というこれまでの形が
正しかった。**挙動は v3.0.1 と変わらない。**

再発を素通りさせたテストも直した。「?」の送出はレポート列が単純化の前後で
完全に一致してしまい、違いはレポート間のウェイトだけだったので、時刻を見る
テストを足した (`tests/test_kana_qmark.cpp`)。
