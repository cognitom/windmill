...
かなレイヤーで <kbd>Win</kbd>(つ) と <kbd>Alt</kbd>(さ) をホールドしている間も、
Ctrlと同じく英数レイヤーへ一時的に移るようにした (issue #34)。かな入力中でも
<kbd>Win</kbd>+<kbd>Q</kbd> や <kbd>Alt</kbd>+<kbd>F</kbd> が英字配列のまま打てる。
タップ (つ / さ) は従来どおりで、キー配置そのものは変わらない。

実装で引っかかったのはレイヤーの下ろしかた。`process_record_kb()` に渡るキーコードは
押下・解放のどちらも**その時点のレイヤー**で引き直されるため、自分で英数レイヤーを
上げてしまうと、解放は `LGUI_T(KC_Z)` ではなく英数レイヤー側の素の `KC_LGUI` として
届く。mod-tapのキーコードを待って `layer_off()` すると、それが永久に来ず英数レイヤーへ
貼りついたまま戻れなくなる。`MY_LCTL` が同じ罠にかからないのは、英数レイヤーの同じ位置が
透過だから。そこで解放はキーイベントではなく、ホストへ出ている修飾の状態を毎スキャン
見て判断するようにした (`update_hold_layer()`)。キー配置に依存しないので、英数レイヤーの
並びを変えても壊れない。

あわせて、Ctrl・Win・Altの重ね押し用に、レイヤーを上げた修飾をビットで覚える
`hold_layer_on()` / `hold_layer_off()` を入れた。QMKのレイヤーは参照カウントを持たないので、
素のままだと <kbd>Ctrl</kbd>+<kbd>Win</kbd> で先に離したほうの `layer_off()` で、
まだ押している側のぶんまでレイヤーが落ちてしまう。

かなレイヤーの「な」「す」「゛」をShiftと一緒に押したとき、親指Shiftのホールドが
確定した直後の1打鍵目だけShiftが乗ったまま解釈されていたのを直した (issue #36)。
「な」なら「ほ」ではなく `S(KC_MINS)` の出力である「ー」が出ていた。

`process_shift_pair()` には、押されているShiftをそのまま使う経路と、いったん実Shiftを
外してから送る経路の2つがあり、issue #18 で直ったのは前者だけだった。後者はレポート列
自体は1打鍵目も2打鍵目も同じで、詰まっていたのは間隔のほう。別キー割り込みで
ホールドが確定する (`HOLD_ON_OTHER_KEY_PRESS`) ため、1打鍵目は

```
[LSFT] (ホールド確定) → [] (Shiftを外す) → [KC_MINS]
```

が全て同じスキャンで出ていて、IMEがShiftの上げ下げを追い切れていなかった。issue #17 で
