...
`process_kana_qmark()` の「?」は、Shiftを自前で上げ下げしても `tap_code16(S(KC_SLSH))`
に任せても**レポート列が完全に一致する**。違うのはレポートとレポートの間隔だけで、
Shiftのレポートと `KC_SLSH` のレポートが間を置かずに並ぶと、英数へ切り替えた直後の
IMEがShiftを取りこぼして半角「/」が出る。issue #39 の単純化がテストを素通りして
実機で再発したのがこれ (PR #41)。

`test_kana_qmark.cpp` の `waits_between_shift_and_slash` は、`send_keyboard_mock` に
`timer_read32()` を差し込んでレポートの時刻を記録し、この間隔を確かめる。
**ウェイトが意味を持つ処理を書いたら、レポート列と一緒に時刻も固定すること。**
