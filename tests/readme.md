...
issue #34 の「Win/Altホールド中は英数レイヤーへ」も、`layer_state` を直接覗かずに
**そのレイヤーでしか出ないキー**を打って判定する (`test_hold_layer.cpp`)。
「ぬ」の位置はかなレイヤーでは `KC_1`、英数レイヤーでは `KC_Q` なので、
ホールド中に打って `[LGUI, KC_Q]` が出るかを見ればレイヤーが分かる。

内部状態ではなくホストへの出力で見ているので、レイヤーの上げ下げをどう実装しても
テストは書き換えずに済む。実際この issue の症状 (ホールド後に英数レイヤーから
戻れない) は、`layer_off()` を呼んでいるつもりでも解放イベントのキーコードが
英数レイヤー側で解決されていて呼ばれていない、という食い違いだった。
...
| `test_hold_layer.cpp` | Ctrl / Win(つ) / Alt(さ) のホールド中だけ英数レイヤーへ移ること |
